...
switch($act)
{
   case 'save':
		...
		if (isset($_FILES['img_url']) && $_FILES['img_url']['tmp_name'] != '') {
			...
			if ($goods_thumb != '' && ctype_digit($_POST['id'])) {
				$sql = 'select img_url from info_lm1 where id_lm=' . $_POST['id'];
		...
		if (isset($_FILES['simg_url']) && $_FILES['simg_url']['tmp_name'] != '') {
			...
			if ($sgoods_thumb != '' && ctype_digit($_POST['id'])) {
				$sql = 'select simg_url from info_lm1 where id_lm=' . $_POST['id'];
				$res = $db->query($sql)->row();
   ...
   case 'del':
		...
		$sql = "delete from info_lm1 where id_lm in($dels)";
		$db->query($sql);
		
		$sql = "select img_sl from info_co1 where lm in($dels)";
		$result = $db->query($sql)->result();
		foreach ($result as $rs)
		{
			if ($rs->img_sl <> '' &&  file_exists(ROOT_PATH.$rs->img_sl)) unlink(ROOT_PATH.$rs->img_sl);
		}
		$sql = "delete from info_co1 where lm in($dels)";
		$db->query($sql);
