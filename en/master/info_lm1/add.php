...
<?php
//添加信息时显示栏目过程
$sql="select * from info_lm1 where fid=0 and xia=1 order by px desc";
...
function dis_child($fid,$i){
	$i .= '&nbsp;&nbsp;';
	global $db;
	$sql='select * from info_lm1 where fid='. $fid .' and xia=1 order by px desc';
    $result = $db->query($sql)->result('array');
...
<li><label>英文名称</label><input name="title_en" type="text" class="dfinput" /></li>
 ...
 <li><label>高级选项</label><cite>  <input name="gao" type="checkbox" id="gao" value="yes" onClick="checkgao()">
	...
	<div id="gaoji" style="display:none;">
      ...
      <tr>
        <td height="30"><label>
          <input name="xia" type="radio" id="xia" value="1" checked>
          是
          <input type="radio" name="xia" id="xia" value="0">
        否 有下级栏目</label></td>
      </tr>
      <tr>
        <td height="30"><label>
    ...
    <tr>
        <td height="30">
          <input name="cpcs" type="radio" id="cpcs" value="1" >
          是
          <input name="cpcs" type="radio" id="cpcs" value="0" checked>
        否 有产品参数</td>
      </tr>
    </table>
	</div>
