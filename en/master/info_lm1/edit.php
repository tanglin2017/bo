...
function dropImg(id)
{
    $.ajax({
        type: 'GET',
        url: "../ajax.php",
        data: {act : "del_infolm1_img", id : id},
...
function dropsImg(id)
{
    $.ajax({
        type: 'GET',
        url: "../ajax.php",
        data: {act : "del_infolm1_simg", id : id},
        success: function(data){  
            if(data.error == 0)
            {
                $("#simg_url_box").empty();
...
$sql = "select * from info_lm1 where id_lm=".$id."";
$query = $db->query($sql);
if ($query->num_rows() == 0)
{
...
document.form1.fid.value=<?php echo $res->fid;?>;
    ...
    <li><label>英文名称</label><input name="title_en" value="<?php echo $res->title_en;?>" type="text" class="dfinput" /></li>
	...
	<div id="gaoji" style="display:none;">
      ...
      <tr>
        ...
        否 有缩略图片二</td>
                  ...
                  <tr>
        <td height="30">
          <input name="cpcs" type="radio" id="cpcs" value="1"  <?php if($res->cpcs) echo 'checked' ?>>
          是
          <input name="cpcs" type="radio" id="cpcs" value="0"  <?php if(!$res->cpcs) echo 'checked' ?>>
        否 有产品参数</td>
      </tr>
    </table>
	</div>
