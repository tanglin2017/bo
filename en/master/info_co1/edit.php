...
<body>
	...
	<?php
$sql = "select * from info_co1 where id=".$id."";
$query = $db->query($sql);
if ($query->num_rows() == 0)
{
...
$res = $query->row();
$sql = 'select * from info_lm1 where id_lm=' . $res->lm; 
...
$('#lm').val(<?php echo $res->lm;?>)
	...
	<textarea class="dfinput" id="keywords" name="title_key" style="width:345px;height:70px;"><?php echo $res->title_key;?></textarea>
	</li>
   
    <li id="dis_key2"><label>网站描述</label><textarea class="dfinput"  id="title_tion" name="title_tion" style="width:345px;height:100px;"><?php echo $res->title_tion;?></textarea></li>
    
	
	
	<li id="dis_cpcs" <?php if(!$dis_lm->cpcs) echo "style='display:none;'"; ?>><label>产品参数</label><textarea  id="cpcs" name="cpcs" style="width:800px;height:200px;visibility:hidden;"><?php echo htmlspecialchars($res->cpcs,ENT_QUOTES);?></textarea></li>
...
<li id="dis_z_body" <?php if(!$dis_lm->z_body) echo "style='display:none;'"; ?> ><label>文章内容</label><textarea  id="z_body" name="z_body" style="width:800px;height:400px;visibility:hidden;"><?php echo htmlspecialchars($res->z_body,ENT_QUOTES);?></textarea></li>
	...
	<li id="dis_img_sl_en" <?php if(!$dis_lm->bimg_sl) echo "style='display:none;'"; ?>><label>略缩图片(英)</label> 
	...
	<?php 
			...
			?>
			  <div id="bimg_sl_box" >
				<div style="float:left; text-align:center; border: 1px solid #DADADA; margin: 4px; padding:2px;">
				<a href="javascript:;" onClick="if (confirm('您确实要删除该图片吗？')) dropbImg('<?php echo $res->id; ?>')" title="点击删除">[-]</a><br>
	...
	<li > 
        ...
        ?>               
        				...
        				<a href="javascript:;" onClick="if (confirm('您确实要删除该图片吗？')) dropAlbumImg('pro_album','<?php echo $rs->id; ?>',this)" title="点击删除">[-]</a><br>
        				<a href="../../<?php echo $rs->img_url; ?>" target="_blank">
        				<img src="../../<?php echo $rs->img_url; ?>" width="120" height="60"/></a>				</div>		
        <?php
        endforeach;
        ?>                                                           
