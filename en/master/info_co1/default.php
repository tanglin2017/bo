...
if ($id_lm != NULL)
{ 
	$sql="select id_lm,title_lm,fid from info_lm1 where id_lm=".$id_lm." order by px desc";
	$query = $db->query($sql);
	$row = $query->row('array');
	...
	if($fid != 0)
	{
		$sql="select id_lm,title_lm,fid from info_lm1 where id_lm=".$fid." order by px desc";
		$query = $db->query($sql);
		$row = $query->row('array');
		$numrows = $query->num_rows();
		if ($numrows != 0)
...
{
	$sql = "select id_lm,title_lm,fid from info_lm1 where fid=0  order by px desc";
}
else
{
	$sql = "select id_lm,title_lm,fid from info_lm1 where fid=".$id_lm." order by px desc";
}
...
?>
   ...
   $sousuo=$_GET['sousuo'];
    ?>
    	<ul class="toolbar">
        <li class="click"><a href="add.php"><span><img src="../images/t01.png" /></span>添加产品</a></li>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <li style="margin: 0;padding: 0;">
        <form action="default.php" method="get">
        <input type="text" name="sousuo" value="<?php echo $sousuo?>" style="height: 33px;"/>
        <input type="submit" value="搜索产品" style="height: 33px;"/>
        </form></li>
        </ul>
...
function get_child($fid)
{
	static $child = array();
	global  $db;
	$sql = "select id_lm from info_lm1 where fid=$fid";
	$result = $db->query($sql)->result('array');
...
// 获取总数量
if($sousuo!=null){
$sql2="select id from info_co1 where title like '%{$sousuo}%'";
}else {
if($id_lm!=null){
$sql2="select a.*, b.title_lm,b.fid from info_co1 a LEFT JOIN info_lm1 b  ON a.lm=b.id_lm WHERE 1=1" . $tt . "";
}else{
$sql2="select id from info_co1";
}
}
...
if($sousuo!=null){
$sql="SELECT a.*, b.title_lm,b.fid from info_co1 a LEFT JOIN info_lm1 b  ON a.lm=b.id_lm WHERE 1=1 and title like '%{$sousuo}%' ORDER BY a.id DESC limit ".($page-1)*($PageSize).",$PageSize";
}else {
$sql="SELECT a.*, b.title_lm,b.fid from info_co1 a LEFT JOIN info_lm1 b  ON a.lm=b.id_lm WHERE 1=1" . $tt . " ORDER BY a.id DESC limit ".($page-1)*($PageSize).",$PageSize";
}
