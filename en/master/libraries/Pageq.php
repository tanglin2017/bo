...
class pages{
	...
	function __construct($page_size = 20,$rs_total,$query=""){
	...
	public function page_html($page_info){
		if (!empty($page_info['first'])){
			if ($this->page=="" || $this->page==1){
				...
				$page_html	=	" <span class='page_first'><a href='?".$this->query.$this->url['first']."'>".$page_info['first']."</a></span> ";
			}
		}//首页
		...
		if (!empty($page_info['pro'])){
			if ($this->page=="" || $this->page==1){
				...
				$page_html	.=	" <span class='page_pro'><a href='?".$this->query.$this->url['pro']."'>".$page_info['pro']."</a></span> ";
			}
		}//上一页
		...
		if (!empty($page_info['next'])){
			if ($this->page >= $this->page_total){
				...
				$page_html	.=	" <span class='page_next'><a href='?".$this->query.$this->url['next']."'>".$page_info['next']."</a></span> ";
			}
		}//下一页
		...
		if (!empty($page_info['last'])){
			if ($this->page >= $this->page_total){
				...
				$page_html	.=	" <span class='page_last'><a href='?".$this->query.$this->url['last']."'>".$page_info['last']."</a></span>";
			}
		}//最后一页
        ...
        $page_htmls="";
        //跳转
        if (!empty($page_info['select_total'])) {
            $page_htmls .= "<select name='topage' size='1' style='width:40px;' onchange='window.location=this.value'>";
            for($i=1;$i<=$this->page_total;$i++){
                $page_htmls.="<option value='?" . $this->query . "page=".$i. "'>$i</option>";
            }
            $page_htmls .= "</select>";
            
            
            $page_html .= " <span class='select_total'>" . str_replace("%i%", $page_htmls, $page_info['select_total']) . "</span> ";
