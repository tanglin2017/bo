...
<?php
//添加信息时显示栏目过程
$sql="select * from info_lm where id_lm=2 and xia=1 order by px";
...
document.form1.fid.value=<?php echo $res->fid;?>;
    ...
    <li <?php if($id!=2){?>style="display: none;"<?php }?>><label>图片标题</label><input name="title_en" value="<?php echo $res->title_en;?>" type="text" class="dfinput" /></li>
    <li <?php if($id!=2){?>style="display: none;"<?php }?>><label>图片简介</label><textarea class="dfinput" id="jianjie" name="jianjie" style="width:345px;height:50px;"><?php echo htmlspecialchars($res->jianjie,ENT_QUOTES);?></textarea></li>
    <li <?php if($id!=2){?>style="display: none;"<?php }?>><label>栏目图片</label>
    <?php 
			if($res->img_url != '')
			{
			?>
    ...
    <li <?php if($id!=8){?>style="display: none;"<?php }?>><label>栏目内容</label><textarea  id="neirong" name="neirong" style="width:800px;height:200px;visibility:hidden;"><?php echo htmlspecialchars($res->neirong,ENT_QUOTES);?></textarea></li>
