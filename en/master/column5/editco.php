...
<body>
    ...
    <li><a href="../column5/defaultco.php">信息列表</a></li>
    </ul>
    </div>
...
$('#lm').val(<?php echo $res->lm;?>)
	...
	<li id="dis_f_body" <?php if(!$dis_lm->f_body) echo "style='display:none;'"; ?>><label>文章内容</label><textarea  id="f_body" name="f_body" style="width:800px;height:400px;visibility:hidden;"><?php echo htmlspecialchars($res->f_body,ENT_QUOTES);?></textarea></li>
	
	<li id="dis_uselink" <?php if(!$dis_lm->uselink) echo "style='display:none;'"; ?>><label>标题</label><input name="linkurl" type="text" class="dfinput" value="<?php echo $res->linkurl ?>" /></li>
	...
	<span id="jycc33">(建议最佳图片尺寸：<?php if($lm==15){?>550*400<?php }elseif ($lm==16){?>600*433<?php }elseif ($lm==17){?>572*374<?php }?>)</span>
    ...
    <li> 
        ...
        <?php 
        $sql = "select * from pro_album where pro_id=$res->id order by px";
	...
	<li id="dis_info_from" <?php if(!$dis_lm->info_from) echo "style='display:none;'"; ?>><label>多图展示</label> 
		<table id="img_list">
        <tr >
        <td><a  href="javascript:;" onClick="addImg()">[增加]</a><input placeholder="图片标题" name="gallery_desc[]" type="text" size="40" class="dfinput"><input type="file" name="gallery_file[]" class="dfinput55">排序<input value="100" name="gallery_px[]" type="text" style="width: 50px;" class="dfinput"><span>(建议最佳图片尺寸：392*233)</span></td>
        </tr>
        </table> 
    </li>
