...
if($tel==null){
    exit("<script>alert('Failed to add message!');history.back();</script>");
}
//判断邮箱格式是否正确
if(!empty($email)){
    if(!is_mail($email)){
        exit("<script>alert('Mailbox format error!');history.back();</script>");
    }
}
...
$num = 0;
    ...
    if ((int)$num >= $dayMax) {
        ...
        exit("<script>alert('No refresh!');history.back();</script>");
    }
...
msgbox("Message submitted successfully!","parent.location='contact.php'");
exit();
?>
