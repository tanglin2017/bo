...
switch($act)
{
    case 'save':  //保存和修改
		...
		if(isset($_FILES['img_sl']) && $_FILES['img_sl']['tmp_name'] != '')
		{
			...
			{
				$sql = 'select img_sl from info_co where id=' . $_POST['id'];
                ...
                if(isset($_FILES['bimg_sl']) && $_FILES['bimg_sl']['tmp_name'] != '')
		{
			...
			{
				$sql = 'select bimg_sl from info_co where id=' . $_POST['id'];
				$res = $db->query($sql)->row();
		...
		if (isset($_POST['id']) && ctype_digit($_POST['id'])) {
            ...
            if (isset($_FILES['gallery_file'])) {
                ...
                gallery_imgpro($_POST['id'], $_FILES['gallery_file'], $_POST['gallery_desc'],$_POST['gallery_px']);
                gallery_upimgpro($_POST['gallery_id'],$_POST['gallery_updesc'],$_POST['gallery_uppx']);
            }
            msgbox("成功修改!", "location='defaultco.php?id_lm={$lm}';");
        } else {
            $id = $news->save();
            ...
            if (isset($_FILES['gallery_file'])) {
                $image = new Image();
               gallery_imgpro($id, $_FILES['gallery_file'], $_POST['gallery_desc'],$_POST['gallery_px']);
               //gallery_imgpros($id, $_FILES['gallery_file'], $_FILES['gallery_files'], $_POST['gallery_desc']);
            }
            msgbox('', "if (confirm('添加成功,确定要继续添加吗?')) {location='addco.php?lm={$lm}';} else {location='defaultco.php?id_lm={$lm}';}");
        }
		 break;
	...
	case 'ping1':
		...
		{
			$sql="update info_co set pass=0 where id=".$opid."";
			$db->query($sql,false);
		}
		break;  
	case 'ping2':
		...
		{
			$sql="update info_co set pass=1 where id=".$opid."";
			$db->query($sql,false);
		}
		break; 
	case 'tuijian1':
		...
		{
			$sql="update info_co set tuijian=0 where id=".$opid."";
			$db->query($sql,false);
		}
		break;  
	case 'tuijian2':
		...
		{
			$sql="update info_co set tuijian=1 where id=".$opid."";
			$db->query($sql,false);
		}
		break; 
	case 'hot1':
		...
		{
			$sql="update info_co set hot=0 where id=".$opid."";
			$db->query($sql,false);
		}
		break;  
	case 'hot2':
		...
		{
			$sql="update info_co set hot=1 where id=".$opid."";
			$db->query($sql,false);
		}
		break; 
	case 'del':  //删除
		...
		if($opid != '')
		{
			  ...
			  //删除信息包含的图片文件
			  $sql = "select img_url,bimg_url from pro_album where pro_id in($opid)";
			  ...
			  $sql = "delete from pro_album where pro_id in($opid)";
			  $db->query($sql, false);
			  $sql = "select img_sl,bimg_sl from info_co where id in($opid)";
			  $result = $db->query($sql)->result();
			  foreach ($result as $rs)  //如果附件不为空，就删除上传附件的内容
			  {
			  ...
			  $sql = "delete from info_co where id in($opid)";
			  $db->query($sql, false);
		}
		break;    
   default:
