...
<?php
//添加信息时显示栏目过程
$sql="select * from info_lm where id_lm=4 and xia=1 order by px";
...
document.form1.fid.value=<?php echo $res->fid;?>;
    ...
    <li <?php if($id!=4){?>style="display: none"<?php }?>><label>图片标题</label><input name="title_en" value="<?php echo $res->title_en;?>" type="text" class="dfinput" /></li>
    <li <?php if($id!=4){?>style="display: none"<?php }?>><label>栏目图片</label>
    <?php 
			if($res->img_url != '')
			{
			?>
