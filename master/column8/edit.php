...
<?php
//添加信息时显示栏目过程
$sql="select * from info_lm where id_lm=8 and xia=1 order by px";
...
document.form1.fid.value=<?php echo $res->fid;?>;
    ...
    <li <?php if($id!=8){?>style="display: none;"<?php }?>><label>英文名称</label><input name="title_en" value="<?php echo $res->title_en;?>" type="text" class="dfinput" /></li>
    <li <?php if($id!=8){?>style="display: none;"<?php }?>><label>栏目图片</label>
    <?php 
			if($res->img_url != '')
			{
			?>
    ...
    <li <?php if($id!=8){?>style="display: none;"<?php }?>><label>栏目关键字</label><textarea class="dfinput" id="keywords" name="keywords" style="width:345px;height:70px;"><?php echo $res->keywords;?></textarea></li>
    <li <?php if($id!=8){?>style="display: none;"<?php }?>><label>栏目描述</label><textarea class="dfinput"  id="description" name="description" style="width:345px;height:100px;"><?php echo $res->description;?></textarea></li>
