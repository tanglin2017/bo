...
<?php
//添加信息时显示栏目过程
$sql="select * from info_lm where id_lm=1 and xia=1 order by px";
...
document.form1.fid.value=<?php echo $res->fid;?>;
    ...
    <li <?php if($id!=24){?>style="display: none;"<?php }?>><label>数据年份</label><input name="title_en" value="<?php echo $res->title_en;?>" type="text" class="dfinput" /></li>
    <li <?php if($id!=24){?>style="display: none;"<?php }?>><label>背景图片</label>
    <?php 
			if($res->img_url != '')
			{
			?>
    ...
    <li style="display: none;"><label>栏目关键字</label><textarea class="dfinput" id="keywords" name="keywords" style="width:345px;height:70px;"><?php echo $res->keywords;?></textarea></li>
    <li style="display: none;"><label>栏目描述</label><textarea class="dfinput"  id="description" name="description" style="width:345px;height:100px;"><?php echo $res->description;?></textarea></li>
	<li <?php if($id==23 || $id==24 || $id==25){?>style="display: block;"<?php }else {?>style="display: none;"<?php }?>><label>栏目简介</label><textarea  id="jianjie" name="jianjie" style="width:800px;height:200px;visibility:hidden;"><?php echo htmlspecialchars($res->jianjie,ENT_QUOTES);?></textarea></li>
    <li style="display: none;"><label>栏目内容</label><textarea  id="neirong" name="neirong" style="width:800px;height:400px;visibility:hidden;"><?php echo htmlspecialchars($res->neirong,ENT_QUOTES);?></textarea></li>
