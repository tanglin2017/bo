...
if ($id_lm != NULL)
{ 
	$sql="select id_lm,title_lm,fid from info_lm where id_lm=".$id_lm." order by px desc";
	$query = $db->query($sql);
	$row = $query->row('array');
	...
	if($fid != 0)
	{
		$sql="select id_lm,title_lm,fid from info_lm where id_lm=".$fid." order by px desc";
		$query = $db->query($sql);
		$row = $query->row('array');
		$numrows = $query->num_rows();
		if ($numrows != 0)
...
{
	$sql = "select id_lm,title_lm,fid from info_lm where fid=0  order by px desc";
}
else
{
	$sql = "select id_lm,title_lm,fid from info_lm where fid=".$id_lm." order by px desc";
}
...
if($sousuo!=null){
$sql="SELECT a.*, b.title_lm,b.fid from info_co a LEFT JOIN info_lm b  ON a.lm=b.id_lm WHERE 1=1 and title like '%{$sousuo}%' ORDER BY a.id DESC limit ".($page-1)*($PageSize).",$PageSize";
}else {
