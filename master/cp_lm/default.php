...
global $db;
//栏目首页显示栏目过程
$sql = "select * from cp_lm where fid=0 order by px desc";
$db->query($sql);
...
function child($fid,$i){
	$i .= "&nbsp;&nbsp;";
	global $db;
	$sql = "select * from cp_lm where fid=".$fid." order by px desc";
	$result = $db->query($sql)->result('array');
...
?>
  <tr >
    ...
    <td height="22" align="center"><A href="edit.php?id=<?php echo ($rows["id_lm"]);?>">修改设置</A> | <A href="action.php?act=del&id=<?php echo ($rows["id_lm"]);?>"  onClick="return confirm('提示:真的要删除该栏目吗?属于该栏目的所有信息将被删除')">删除</A></td>
  </tr>
<?php
		if ($rows["xia"] == 1) {
			child($rows["id_lm"], $i);
