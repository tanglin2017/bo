...
$retn=$db->query($sqln)->row('array');
?>
        <div class="crumbs f-cb">当前位置： <a href="index.php"><?php echo $retn['title_lm']?></a> &gt; 
<?php 
        ...
        <article class="widget-newsdetail-8 fix">
            <div class="n_left">
                <header class="_head">
                    ...
                    <div class="_tools fix"> <span class="_tool">时间：<?php echo date("Y.m.d",$wtime)?></span> <span class="_tool" id="switcher"> <i class="_title">来源：<?php echo $rsguanjian['e_title']?></i> </span> </div>
                </header>
                <div class="myart fix">
...
?>
<?php if ($rsup){?><a class="next" href="new1.php?id=<?php echo $rsup['id']?>&lm=<?php echo $rsup['lm']?>">下一篇：<?php echo $rsup['title']?></a><?php }else {echo '<a class="next">下一篇：无</a>';}?>
...
?>      
<?php if ($rsdn){?><a class="previous_no" href="new1.php?id=<?php echo $rsdn['id']?>&lm=<?php echo $rsdn['lm']?>">上一篇：<?php echo $rsdn['title']?></a><?php }else {echo '<a class="previous_no">上一篇：无</a>';}?>   

            
            </div>
            <div class="n_right">
                <div class="_tools fix"> <span class="_tool _back"><a href="javascript:history.go(-1);">返回列表</a></span></div>
            </div>
        </article>
    </div>
</section>
